...
async def call_scraper(funcStr, url):
    #TODO:
    #handle only ids being sent, append dotabuff website to it
    #use regex
    ...
    async with ClientSession(headers=headers) as session:
        ...
        async def return_matchids(url, session, lim=5):
        ...
        async def get_match_info(id, session):
            ...
            for i in range(0,2):
                ...
                xydata.append(xdata)
            return xydata
        if (funcStr == "get_match_info"):
            return await get_match_info(url, session)
        elif (funcStr == "parse_profile"):
            return await parse_profile(url, session)
        elif (funcStr == "fetch_matches"):
            #param needs id list
            return await fetch_matches(url, session)
